...
Roles, approval protocol, model selection, and pre-work checklist are defined in [`.claude/GOVERNANCE.md`](./.claude/GOVERNANCE.md). Read it before delegating to sub-agents or starting multi-file tasks.

**Persona pre-work is self-enforced** (no hook): for architectural or security-sensitive files, load the relevant persona doc from `.claude/personas/` before editing. The checklist is in GOVERNANCE.md.
...
Always create worktrees in `.worktrees/` at the project root. **Never use `.claude/worktrees/`.**
...
When using `EnterWorktree` or any native worktree tool, verify the target path before creation. If the tool defaults to `.claude/worktrees/`, override it or use `git worktree add .worktrees/<name> -b <name>` directly.
...
- **Advisory hooks**: 컨텍스트 주입만. 차단하지 않음.

### Final-Stage Gates (커밋 시점, deny)

| Hook | Behavior | Trigger |
|------|----------|---------|
| `guard-commit-message.sh` | **Deny** — Conventional Commits 형식 강제 | 커밋 메시지 형식 위반 |
| `guard-code-review.sh` | **Deny** — `/code-review` 결과 필수 (Critical=0, Major≤2, TTL 60분) | 리뷰 없음 또는 미통과 |
| `guard-static-analysis.sh` | **Deny** — Checkstyle+PMD 위반 시 차단 | staged main Java 파일에 위반 있을 때 |

`guard-code-review.sh`는 Java production 코드 변경이 없는 커밋(docs, config 등)에서는 자동 통과합니다.

### Advisory Hooks (차단 없음)

| Hook | Event | Purpose |
|------|-------|---------|
| `session-start-context.sh` | SessionStart | 미테스트 클래스, 브랜치, TDD 원칙 주입 |
| `persona-inject.sh` | Write/Edit | Entity/Security/Gradle 편집 시 페르소나 컨텍스트 주입 |
| `pre-commit-context.sh` | Bash (commit) | staged 파일 중 테스트 없는 항목 표시 |
| `session-stop-report.sh` | Stop | 세션 위반 요약 |

### Code Review Gate

`/code-review` skill writes `tmp/review-result.json`. The commit hook validates and deletes it.
- Location: `tmp/` (프로젝트 루트, gitignored)
- TTL: 60 minutes — expired review requires re-running `/code-review`
- Threshold: Critical=0, Major≤2 → commit allowed; else denied

### TDD (지침 — 코드리뷰에서 검증)

TDD는 hook이 아닌 지침으로 운영합니다. 코드리뷰 게이트에서 테스트 누락이 검증됩니다.
...
**See `.claude/skills/code-review/references/checkstyle-conventions.md` for fix patterns** (load it before touching violations). Human-readable version: `docs/conventions/checkstyle.ko.md`.
...
**Architecture governance**: When editing Entity, Controller, or Gradle files, `persona-inject.sh` (Write/Edit hook) automatically injects the relevant persona doc from `.claude/personas/` as context before proceeding.
