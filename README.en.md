...
Autonomous Run Ledger is an advisory artifact for long-running workflow continuation. The ledger is written to `.ai/interview/runs/<workflowId>/ledger.json` and stores compact JSON for workflow identity, current phase, plan/task coverage, diff coverage, verification summary, review summary, and next safe action summary. The ledger does not store raw prompts, raw transcripts, full stdout, or full stderr; it records paths, counts, status values, and short summaries. The first MVP does not block phase transitions. It helps restore state after context compaction or restart.
