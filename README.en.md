...
EPIC PEV Queue is an optional task queue for large interviews that produce multiple tasks. It keeps the existing workflow phases unchanged and switches only the active task stored in `WorkflowInstance.taskQueue`. The `workflow_task_queue` tool uses `create`, `add`, `activate`, `mark`, and `show` operations to create a queue, select one active task, and record each task as `pending`, `active`, `done`, `blocked`, or `deferred`. The full queue snapshot is written to `.ai/interview/runs/<workflowId>/task-queue.json`, while prompt context shows only the queue title, active task, status counts, next pending task, and bounded scope/acceptance/verification cues for the active task. The implement-phase autonomous task execution policy uses the active task as the goal lock and guides next action, test decision, failure escape, and completion/switch decisions. Marking the active task `done` automatically activates the next `pending` task without user approval and writes completion evidence plus the queue artifact. If any `active`, `pending`, or `blocked` task remains, the `implement → code_review` transition is blocked to prevent silent partial completion. This feature is not unbounded discovery: out-of-scope findings are recorded as pending or deferred tasks instead of being implemented automatically. Existing single-workflow usage and the `commit → push` approval boundary are unchanged.

Autonomous Run Ledger is an advisory artifact for long-running workflow continuation. The ledger is written to `.ai/interview/runs/<workflowId>/ledger.json` and stores compact JSON for workflow identity, current phase, plan/task coverage, diff coverage, verification summary, review summary, verification coverage matrix, and evidence-based next safe action summary. When an active workflow has a ledger, prompt context can add a bounded `[Run Ledger Resume]` block with only phase, latest verification, coverage count, review count, diff count, and next safe action. The ledger does not store raw prompts, raw transcripts, full stdout, or full stderr; it records paths, counts, status values, and short summaries. Run Ledger does not block phase transitions. It helps restore state after context compaction or restart.
...
Principles: inject only relevant top-N active memories, never inject candidate memory, keep metrics and explain output to ids/hashes/counts/scores/reasons instead of raw prompts, reject raw secret-like memory text, and avoid storing a new entry when the same summary already exists. Planned later: approve/reject workflow, merge/supersede, export, and AGENTS.md promotion.
...
- Context-heavy implementation, code review, large diff analysis, and log analysis should prefer subagents so the main agent remains a workflow controller with minimal context pollution. Pi prompts inject the concise `[WORKFLOW HARD RULES]` block from `.harness/workflow-policy.json` to remind the agent about phase order, approval boundaries, the code-review fix loop, subagent use, and main-context hygiene. `target/.pi/settings.json` raises the reviewer subagent default execution limit to 300000ms so long diff/review fixtures do not repeatedly hit the previous 120s timeout. In `implement`, `code_review`, `document`, and `commit`, prompts also inject the current `[CONTEXT STRATEGY]`: what main keeps, what it avoids, and the expected subagent return format. The context-budget policy is **main-session-as-controller** by default: long reviewer/subagent output, test logs, and diff analysis should be **artifact-first** or `file-only`, while the main transcript keeps a concise summary and artifact path. Workflow prompts include short execution principles: act when enough information is available, and do not re-litigate established decisions. Continuation prompts tell the agent not to stop or suggest a new session because of context limits, and they do not expose token countdown numbers. Workflow command output artifacts are working artifacts stored under `.ai/workflow-artifacts/` and are not tracked by git. Run Ledger artifacts are stored under `.ai/interview/runs/<workflowId>/ledger.json` and keep coverage matrix, verification, review, diff count, and next safe action summaries only. The `[Run Ledger Resume]` prompt block uses the same compact summary. Safety gates and the commit → push approval boundary remain intact. Failures, security issues, and `Critical/Major` review findings must not be hidden.

long-running workflow operations use short heartbeat notifications so the TUI does not look frozen. `workflow_run_command` periodically emits `workflow_run_command <id> still running` while a command is active, and the `implement → code_review` Pre-code_review quality check uses the same heartbeat path. Heartbeats must not repeat raw logs or increase context pollution. Direct heartbeat support for the automatic `code_review → review_approved` quality gate is deferred because `gates.ts` does not receive UI context; that requires a separate gate progress sink design.
