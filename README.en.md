...
Requirements discovery now focuses on the default workflow `interview → plan` path. The separate feature-interview, feature-planning-room, and requirements-room skills and slash commands have been removed. The `interview` skill now follows a deep-interview-lite style: it asks the five LLM-provided baseline questions first (scope, motivation, acceptance criteria, affected files/modules, constraints/risks) and appends only a clarity checkpoint at the end of the initial round. The runtime no longer forces a work-map/topology question to be first. Choices may include optional `recommended` and `recommendationReason` fields. Recommendation display is advisory information, and the user's selected answer is the source of truth. Brownfield work should cite narrow repo evidence first, and follow-up questions target the weakest clarity dimension across goal, scope, acceptance criteria, constraints, and existing context. Low-score follow-up wizard rounds pass `round: 'follow_up'` so they show only targeted questions instead of repeating the clarity checkpoint. After the wizard completes, the LLM must record goal/scope/acceptance/constraints/context scores (0–100 each) with `workflow_score_interview`; any dimension below 60 blocks the `interview → plan` transition.
