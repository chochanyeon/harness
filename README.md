...
Context 예산 정책은 **main-session-as-controller**를 기본값으로 둡니다. 긴 reviewer/subagent 출력, 테스트 로그, diff 분석은 **artifact-first** 또는 `file-only` output을 우선 사용하고 main transcript에는 짧은 요약과 artifact path를 남깁니다. Workflow command output artifact는 `.ai/workflow-artifacts/` 아래의 작업 산출물로 저장되며 git 추적 대상이 아닙니다. Run Ledger artifact는 `.ai/interview/runs/<workflowId>/ledger.json`에 저장되며 coverage matrix, verification, review, diff count, next safe action 요약만 보관합니다. `[Run Ledger Resume]` prompt block도 같은 compact summary만 사용합니다. 단, 실패, 보안 문제, `Critical/Major` review finding은 숨기면 안 됩니다(must not be hidden).
