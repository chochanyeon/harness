...
요구사항 수집은 workflow의 기본 `interview → plan` 흐름에 집중합니다. 별도 feature-interview/planning-room/requirements-room skill과 slash command는 제거했습니다. `interview` skill은 deep-interview-lite 방식으로 동작해, LLM이 제공한 5개 baseline 질문(범위, 동기, 완료 기준, 영향 파일/모듈, 제약/위험)을 그대로 먼저 묻고 첫 라운드 끝에 clarity checkpoint만 자동 추가합니다. Runtime은 더 이상 work map/topology 질문을 첫 질문으로 강제하지 않습니다. Choice는 선택 필드 `recommended`와 `recommendationReason`을 가질 수 있으며, 추천 표시는 advisory 정보입니다. 사용자가 선택한 답변이 source of truth입니다. Brownfield 작업에서는 좁은 repo evidence를 먼저 인용하며, 목표/범위/완료 기준/제약/기존 맥락 중 가장 약한 명확성 차원을 기준으로 후속 질문합니다. 낮은 점수 차원을 다시 묻는 follow-up wizard는 `round: 'follow_up'`으로 실행해 clarity checkpoint를 반복하지 않고 targeted 질문만 표시합니다. Wizard 완료 후 LLM은 `workflow_score_interview`로 목표/범위/완료 기준/제약/기존 맥락 점수(각 0–100)를 기록해야 하며, 한 차원이라도 60 미만이면 `interview → plan` 전이가 차단됩니다.
