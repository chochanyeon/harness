...
Autonomous Run Ledger는 장기 실행 복원을 위한 advisory artifact입니다. Ledger는 `.ai/interview/runs/<workflowId>/ledger.json`에 저장되며 workflow identity, current phase, plan/task coverage, diff coverage, verification summary, review summary, next safe action summary를 compact JSON으로 기록합니다. Ledger는 raw prompt, raw transcript, full stdout, full stderr를 저장하지 않고 path/count/status/short summary 중심으로 기록합니다. 첫 MVP는 phase 전이를 차단하지 않으며, context compaction 또는 restart 후 현재 상태를 복원하는 데 사용합니다.
...
Context 예산 정책은 **main-session-as-controller**를 기본값으로 둡니다. 긴 reviewer/subagent 출력, 테스트 로그, diff 분석은 **artifact-first** 또는 `file-only` output을 우선 사용하고 main transcript에는 짧은 요약과 artifact path를 남깁니다. Workflow command output artifact는 `.ai/workflow-artifacts/` 아래의 작업 산출물로 저장되며 git 추적 대상이 아닙니다. Run Ledger artifact는 `.ai/interview/runs/<workflowId>/ledger.json`에 저장되며 coverage, verification, review, next safe action 요약만 보관합니다. 단, 실패, 보안 문제, `Critical/Major` review finding은 숨기면 안 됩니다(must not be hidden).
