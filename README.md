...
- Autonomous Run Ledger 생성 (`.ai/interview/runs/<workflowId>/ledger.json`)
...
요구사항 수집은 workflow의 기본 `interview → plan` 흐름에 집중합니다. 별도 feature-interview/planning-room/requirements-room skill과 slash command는 제거했습니다. `interview` skill은 deep-interview-lite 방식으로 동작해, LLM이 제공한 5개 baseline 질문(범위, 동기, 완료 기준, 영향 파일/모듈, 제약/위험)을 그대로 묻습니다. Runtime은 작업 범위 확인 질문이나 명확성 확인 질문을 자동 추가하지 않습니다. 필요한 경우 LLM이 baseline 질문 안에 영향 범위, 제외/보류 범위, 불명확한 완료 기준을 직접 묻습니다. Choice는 선택 필드 `recommended`와 `recommendationReason`을 가질 수 있으며, 추천 표시는 advisory 정보입니다. 사용자가 선택한 답변이 source of truth입니다. Brownfield 작업에서는 좁은 repo evidence를 먼저 인용하며, 목표/범위/완료 기준/제약/기존 맥락 중 가장 약한 명확성 차원을 기준으로 후속 질문합니다. 낮은 점수 차원을 다시 묻는 follow-up wizard는 `round: 'follow_up'`으로 실행해 targeted 질문만 표시합니다. Wizard 완료 후 LLM은 `workflow_score_interview`로 목표/범위/완료 기준/제약/기존 맥락 점수(각 0–100)를 기록해야 하며, 점수 evidence는 완료된 wizard 또는 해시된 chat interview artifact와 결합되어야 합니다. 한 차원이라도 60 미만이거나 source evidence가 없으면 `interview → plan` 전이가 차단됩니다.

산출물은 `.ai/interview/` 아래에 저장됩니다. 한국어 `.ko.md` 파일은 사람용 source of truth이고, 영어 `.md` 파일은 DPAA/SBADR 친화적인 machine-check artifact입니다. Spec에는 작업 범위, 영향 범위, 필요한 용어 정의가 포함되고, plan step은 관련 영향 범위 및 acceptance criteria에 매핑됩니다.

EPIC PEV Queue는 큰 인터뷰에서 여러 작업이 나온 경우에만 사용하는 선택적 task queue입니다. 기존 workflow phase는 그대로 유지하고 `WorkflowInstance.taskQueue`의 active task만 전환합니다. `workflow_task_queue` tool은 `create`, `add`, `activate`, `mark`, `show` operation으로 queue를 만들고 하나의 active task를 선택하며, 상태는 `pending`, `active`, `done`, `blocked`, `deferred` 중 하나로 기록됩니다. 전체 queue snapshot은 `.ai/interview/runs/<workflowId>/task-queue.json`에 저장되고, prompt context에는 queue 제목, active task, 상태 count, 다음 pending task, active task의 scope/acceptance/verification 단서만 bounded summary로 요약됩니다. implement 단계의 autonomous task execution policy는 이 active task를 goal lock으로 삼아 next action, test decision, failure escape, completion/switch 판단을 자율적으로 수행하도록 안내합니다. active task를 `done`으로 표시하면 다음 `pending` task가 사용자 승인 없이 자동 활성화되고, 완료 evidence와 queue artifact가 함께 갱신됩니다. `active`/`pending`/`blocked` task가 남아 있으면 silent partial completion을 막기 위해 `implement → code_review` 전이가 차단됩니다. 이 기능은 무제한 discovery가 아니며, 범위 밖 발견은 자동 구현하지 않고 pending/deferred task로 기록합니다. 기존 단일 workflow 사용과 `commit → push` 승인 경계는 변경되지 않습니다.

Autonomous Run Ledger는 장기 실행 복원을 위한 advisory artifact입니다. Ledger는 `.ai/interview/runs/<workflowId>/ledger.json`에 저장되며 workflow identity, current phase, plan/task coverage, diff coverage, verification summary, review summary, verification coverage matrix, evidence-based next safe action summary를 compact JSON으로 기록합니다. Prompt context는 active workflow의 ledger가 있으면 `[Run Ledger Resume]` bounded block을 추가할 수 있으며, block에는 phase, latest verification, coverage count, review count, diff count, next safe action만 들어갑니다. Ledger는 raw prompt, raw transcript, full stdout, full stderr를 저장하지 않고 path/count/status/short summary 중심으로 기록합니다. Run Ledger는 phase 전이를 차단하지 않으며, context compaction 또는 restart 후 현재 상태를 복원하는 데 사용합니다.
...
DPAA 의존성은 첫 DPAA gate 실행 시 `.pi/.venv/`에 자동 설치됩니다. `scripts/sync-dev-harness.py`는 개발 repo 동기화 중 기존 `.pi/.venv/`를 부분 삭제하지 않고 보존합니다. DPAA gate는 venv Python뿐 아니라 pip 실행 가능 여부도 확인하며, pip가 깨졌으면 `ensurepip --upgrade`를 시도하고 필요하면 `.pi/.venv/`를 새로 만들어 복구합니다.
...
Context 예산 정책은 **main-session-as-controller**를 기본값으로 둡니다. 긴 reviewer/subagent 출력, 테스트 로그, diff 분석은 **artifact-first** 또는 `file-only` output을 우선 사용하고 main transcript에는 짧은 요약과 artifact path를 남깁니다. Workflow prompt는 충분한 정보가 있으면 행동하고, 이미 결정된 내용을 다시 논쟁하지 말라는 짧은 실행 원칙을 포함합니다. Continuation prompt는 context limit 때문에 멈추거나 새 session을 제안하지 말라고 안내하며, token countdown 숫자를 노출하지 않습니다. Workflow command output artifact는 `.ai/workflow-artifacts/` 아래의 작업 산출물로 저장되며 git 추적 대상이 아닙니다. Run Ledger artifact는 `.ai/interview/runs/<workflowId>/ledger.json`에 저장되며 coverage matrix, verification, review, diff count, next safe action 요약만 보관합니다. `[Run Ledger Resume]` prompt block도 같은 compact summary만 사용합니다. 안전 gate와 commit → push approval boundary는 유지됩니다. 단, 실패, 보안 문제, `Critical/Major` review finding은 숨기면 안 됩니다(must not be hidden).

long-running workflow 작업은 짧은 heartbeat로 TUI가 살아 있음을 표시합니다. `workflow_run_command`는 command가 실행 중일 때 `workflow_run_command <id> still running` notification을 주기적으로 내고, `implement → code_review` 전 Pre-code_review 품질 검사도 같은 heartbeat 경로를 사용합니다. heartbeat는 raw log를 반복 출력하지 않아 context pollution을 늘리지 않아야 합니다. `code_review → review_approved` 자동 quality gate의 직접 heartbeat는 `gates.ts`가 UI context를 받지 않는 구조라 deferred 상태이며, 별도 gate progress sink 설계가 필요합니다.
...
`commit → push`로 넘어갈 때 push 의사와 위험 변경을 함께 확인합니다. 이때 tracked staged/modified 변경이 남아 있으면 아직 커밋되지 않은 변경이 push에서 누락될 수 있으므로 `Push blocked: uncommitted changes exist.`로 전환을 차단합니다. untracked 파일은 push 대상이 아니므로 이 guard에서는 차단하지 않습니다. Push policy scan은 dirty working tree만 보지 않고 upstream 대비 outgoing commit diff를 우선 검사하며, upstream이 없는 경우에도 마지막 commit 기준으로 보수적으로 검사합니다. git 오류나 root 확인 실패는 fail-open이 아니라 policy scan unavailable finding으로 차단합니다. build/config/migration/Docker/CI/deletion/large-change뿐 아니라 `auth`, `session`, `security`, `secret`, `token`, `permission`, `schema.prisma`, `.env*`, compose 파일, `infra`, `terraform`, `k8s`, `rbac`, `iam`, `policy` 같은 고위험 경로/파일도 별도 category로 표시합니다. `bash`의 `git push`와 `workflow_run_command`의 `git-push` catalog command는 모두 workflow workspace, `commit → push` transition history, policy approval을 확인합니다.
...
| 의존성 | 용도 | 비고 |
|---|---|---|
| Python 3.10+ | DPAA, SBADR | 하네스가 venv 자동 생성, pip 손상 시 복구 |
