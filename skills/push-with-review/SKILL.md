...
- Commit proceeds automatically (invoking this skill counts as approval per CLAUDE.md):
  ```bash
  git add <files>
...
Check for uncommitted changes → propose commit message → then commit automatically (skill invocation = approval per CLAUDE.md):
