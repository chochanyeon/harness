...
1. **TDD 필수**: 구현 파일 작성 전 반드시 XxxTest.java를 먼저 작성하세요.
   - `guard-tdd-write.sh`가 테스트 없이 구현 파일 Write를 차단합니다.
   - 테스트 파일에 @Test 어노테이션이 포함되어야 토큰이 생성됩니다.

2. **커밋 메시지**: Conventional Commits 형식 준수
   - `feat(scope): 설명` 형식
   - `guard-commit-message.sh`가 형식 위반 커밋을 차단합니다.

3. **커밋 전 코드 리뷰**: 커밋 전 `/code-review` 스킬을 실행하세요.
   - `guard-code-review.sh`가 리뷰 없는 커밋을 차단합니다.
...
| 게이트 | 서브 에이전트 영향 |
|--------|-----------------|
| `guard-tdd-write.sh` | 서브 에이전트가 테스트 파일(@Test)을 먼저 Write → 토큰 생성 → 구현 파일 Write 가능 |
| `guard-tdd-edit.sh` | 서브 에이전트가 기존 파일 Edit 시 테스트 변경 없으면 ask — 프롬프트에 "기존 테스트가 커버함" 또는 "새 테스트 필요" 명시 |
| `guard-commit-message.sh` | 서브 에이전트 커밋 메시지에 Conventional Commits 형식 지시 |
| `guard-code-review.sh` | 각 태스크 완료 후 `/code-review` 스킬 실행 지시 |
