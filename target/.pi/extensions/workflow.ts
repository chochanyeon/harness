...
${formatWorkflowAction(state.workflow)}` }],
  ...
  pi.registerTool({
    ...
    description: [
      ...
      "For every round, the tool displays exactly the questions provided by the LLM and adds no automatic scope-map or clarity-check questions. For follow-up rounds, pass round: 'follow_up' to document that the provided questions are targeted follow-ups.",
    ...
    promptGuidelines: [
      "Call workflow_interview_wizard immediately when an interview phase starts. Generate 5 baseline questions (scope, motivation, acceptance criteria, affected files/modules, constraints/risks) with domain-specific choices for the workflow goal. Choices may include optional recommended and recommendationReason fields to mark advisory recommendations; user selection remains the source of truth. The tool adds no automatic questions. If the task needs extra confirmation about affected areas, excluded scope, or unclear criteria, include that question directly in the provided baseline. For low-score follow-ups, pass round: 'follow_up' and ask only targeted follow-up questions."
    ],
    parameters: Type.Object({
