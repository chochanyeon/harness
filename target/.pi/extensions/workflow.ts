...
function buildDeepInterviewLiteQuestions(_workflowTitle: string, baselineQuestions: InterviewQuestion[], round: "initial" | "follow_up" = "initial"): InterviewQuestion[] {
...
${formatWorkflowAction(state.workflow)}` }],
  ...
  pi.registerTool({
    ...
    description: [
      ...
      "For the initial round, the tool appends a deep-interview-lite clarity checkpoint without prepending a topology/work-map question. For follow-up rounds, pass round: 'follow_up' to ask only the provided targeted questions.",
    ...
    promptGuidelines: [
      "Call workflow_interview_wizard immediately when an interview phase starts. Generate 5 baseline questions (scope, motivation, acceptance criteria, affected files/modules, constraints/risks) with domain-specific choices for the workflow goal. Choices may include optional recommended and recommendationReason fields to mark advisory recommendations; user selection remains the source of truth. The initial round appends only a clarity checkpoint automatically; it does not prepend a work-map/topology question. For low-score follow-ups, pass round: 'follow_up' and ask only targeted follow-up questions."
    ],
    parameters: Type.Object({
      ...
      round: Type.Optional(Type.Union([Type.Literal("initial"), Type.Literal("follow_up")], { description: "Use 'initial' for the first interview round; use 'follow_up' to avoid repeating the clarity checkpoint." })),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
