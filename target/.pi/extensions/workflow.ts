...
function buildTopologyQuestion(workflowTitle: string): InterviewQuestion {
  return {
    ...
    title: "Round 0: Work map / scope topology",
    prompt: [
      `작업 '${workflowTitle}'을(를) 시작하기 전에 먼저 작업 지도를 만듭니다.`,
      "Purpose: prevent the plan from missing a major surface, component, integration point, or deferred area.",
      "What to answer: list the main parts that may be touched, what success/failure will be judged against, and any part that should be excluded or deferred.",
      "자유입력에는 관련 컴포넌트/파일군/사용자 흐름/문서/테스트/외부 연동을 짧은 목록으로 적어주세요.",
    ].join("\n"),
    helpText: "The choices describe the rough shape only; use free text for the real answer. 예: 포함: Interview wizard UX, prompt contract tests. 제외/보류: scoring formula 변경.",
    required: true,
    allowFreeText: true,
    allowSkip: false,
    choices: [
      { id: "single_component", label: "한 컴포넌트/파일군만 주로 건드림" },
      { id: "multiple_components", label: "여러 컴포넌트/문서/테스트가 함께 바뀜" },
      { id: "needs_split_merge", label: "범위를 나누거나 합칠 결정이 필요함" },
      { id: "has_deferred_scope", label: "이번에 제외하거나 보류할 범위가 있음" },
      { id: "unsure", label: "아직 작업 지도가 불확실함" },
    ],
  };
}
...
${formatWorkflowAction(state.workflow)}` }],
  ...
  pi.registerTool({
    ...
    description: [
      ...
      "For the initial round, the tool wraps those questions with deep-interview-lite work-map/topology confirmation and a clarity checkpoint. For follow-up rounds, pass round: 'follow_up' to ask only the provided targeted questions.",
    ...
    promptGuidelines: [
      "Call workflow_interview_wizard immediately when an interview phase starts. Generate 5 baseline questions (scope, motivation, acceptance criteria, affected files/modules, constraints/risks) with domain-specific choices for the workflow goal. The initial round adds a work-map/topology question and clarity checkpoint automatically. For low-score follow-ups, pass round: 'follow_up' and ask only targeted follow-up questions.",
    ],
    parameters: Type.Object({
      ...
      round: Type.Optional(Type.Union([Type.Literal("initial"), Type.Literal("follow_up")], { description: "Use 'initial' for the first interview round; use 'follow_up' to avoid repeating the topology and clarity scaffold." })),
    }),
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
