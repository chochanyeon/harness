...
export function createWorkflowContinuationService(state: WorkflowRuntimeState, pi: ExtensionAPI) {
  ...
  function buildPrompt(workflow: WorkflowInstance, marker: string): string {
    return [
      ...
      "",
      "Rules:",
      "- Continue within the current phase. Preserve guard semantics and the phase-specific required action.",
      "- Context budget: keep main as controller. Do not stop, summarize, or suggest a new session on account of context limits.",
      "- Prefer short summaries and artifact paths for long evidence.",
      "- Approval boundary: only commit → push needs the user. Other transitions are autonomous. Do not ask the user for permission to re-enter a review phase.",
      "- Guards stay mandatory: DPAA, SBADR, submit_review_package, quality, policy, and workspace.",
      "- plan_review repair: high-risk (Risk: high, Ambiguity gate: strict, or Work type: api/security/migration/data/deploy) plans need Architect/Critic consensus review; DPAA/SBADR failure means return to plan, fix, and retry.",
      "- code_review repair: quality or review failure means return to implement, fix, and retry.",
      "",
