...
export const COMMAND_CATALOG: readonly CommandSpec[] = [
  ...
  {
    id: "git-reset-soft-to-checkpoint-base",
    description: "Reset to this workflow's recorded implement-entry checkpoint base with git reset --soft, keeping all changes staged. Target commit comes from workflow state, not from args.",
    executable: "git",
    fixedArgs: ["reset", "--soft"],
    allowedPhases: ["commit"],
    cwdPolicy: "git-root",
    timeoutMs: 15_000,
    maxOutputBytes: 10_000,
    outputPolicy: "inline",
  ...
  {
    id: "git-reset-hard-to-checkpoint",
    description: "Roll back to an earlier implement-phase checkpoint commit with git reset --hard. Supply the target commit hash via args: [\"<hash>\"]. Rejected unless the hash is a descendant of this workflow's recorded checkpoint base.",
    executable: "git",
    fixedArgs: ["reset", "--hard"],
    allowUserArgs: true,
