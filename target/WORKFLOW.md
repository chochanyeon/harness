...
|---------|------|------|----------|
| Commit message gate | `.pi/extensions/workflow.ts` | `git commit` | Commit Format |
| Code review gate | `.pi/extensions/workflow.ts` | `git commit` | Code Review |
| Workflow state layer | `.pi/extensions/workflow.ts` | `/workflow *` | Planning → Push |
| Review token tool | `.pi/extensions/workflow.ts` | `submit_review_result` | Code Review |
