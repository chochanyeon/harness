...
**Architecture:** INDEX.md + Claude 판단 방식. Claude는 작업 시작 전 INDEX.md를 읽고 관련 파일을 선택적으로 로드한다. PostToolUse 훅은 편집 파일 경로와 메모리 파일의 태그를 매칭해 핵심 규칙을 주입한다.
...
| `.claude/hooks/post-tool-use-memory.js` | PostToolUse 훅 (태그 매칭 → 리마인더) | 신규 생성 |
| `CLAUDE.md` | 작업 전 INDEX 읽기 지시사항 | 수정 |
| `.claude/settings.json` | 훅 등록 | 수정 |
...
## Task 2: CLAUDE.md에 Project Memory 지시사항 추가

**Files:**
- Modify: `CLAUDE.md`

- [ ] **Step 1: CLAUDE.md의 `## Core Execution Rules` 섹션 앞에 추가**

다음 섹션을 `CLAUDE.md`의 `## Core Execution Rules` 바로 위에 삽입:
...
```bash
git add CLAUDE.md
git commit -m "feat(memory): CLAUDE.md에 Project Memory 읽기 지시사항 추가"
```
...
**Files:**
- Create: `.claude/hooks/post-tool-use-memory.js`

- [ ] **Step 1: 훅 파일 생성**

`.claude/hooks/post-tool-use-memory.js`:
```javascript
#!/usr/bin/env node
'use strict';
...
```bash
echo '{"tool_name":"Write","tool_input":{"file_path":"D:/JavaProject/DevCenter/src/main/java/com/example/service/UserService.java"}}' | node .claude/hooks/post-tool-use-memory.js
```
...
```bash
echo '{"tool_name":"Write","tool_input":{"file_path":"D:/JavaProject/DevCenter/docs/README.md"}}' | node .claude/hooks/post-tool-use-memory.js
```
...
```bash
echo '{"tool_name":"Read","tool_input":{"file_path":"D:/JavaProject/DevCenter/src/main/java/UserService.java"}}' | node .claude/hooks/post-tool-use-memory.js
```
...
```bash
git add .claude/hooks/post-tool-use-memory.js
...
`.claude/settings.json`의 `"PostToolUse"` 배열에 다음 항목 추가:
  ...
  "hooks": [
    {
      "type": "command",
      "command": "node",
      "args": ["${CLAUDE_PROJECT_DIR}/.claude/hooks/post-tool-use-memory.js"]
    }
  ]
}
```
    ...
    "hooks": [
      {
        "type": "command",
        "command": "node",
        "args": ["${CLAUDE_PROJECT_DIR}/.claude/hooks/code-guardrail.js"]
      }
    ]
  },
  {
    ...
    "hooks": [
      {
        "type": "command",
        "command": "node",
        "args": ["${CLAUDE_PROJECT_DIR}/.claude/hooks/validate-feat-html.js"]
      }
    ]
  },
  {
    ...
    "hooks": [
      {
        "type": "command",
        "command": "node",
        "args": ["${CLAUDE_PROJECT_DIR}/.claude/hooks/validate-feat-index.js"]
      }
    ]
  },
  {
...
```bash
echo '{"tool_name":"Edit","tool_input":{"file_path":"D:/JavaProject/DevCenter/src/main/java/com/example/controller/UserController.java","old_string":"","new_string":""}}' | node .claude/hooks/post-tool-use-memory.js
```
